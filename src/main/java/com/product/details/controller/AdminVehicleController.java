...
public class AdminVehicleController {
	...
	@RequestMapping(value = "/insertMotorcycleProduct", method = RequestMethod.POST)
	public ResponseEntity<JSONObject> insertMotorcycleProductMethod(@RequestBody ProductModel pm){
		JSONObject retJs = new JSONObject();
		try {
			
			int insertFlag = productService.insertMotorcycleProduct(pm);
			if(insertFlag > 0){
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/updateMotorcycleProduct", method = RequestMethod.POST)
	public ResponseEntity<JSONObject> updateMotorcycleProductMethod(@RequestBody ProductModel pm){
		JSONObject retJs = new JSONObject();
		try {
			
			int insertFlag = productService.updateMotorcycleProduct(pm);
			if(insertFlag > 0){
